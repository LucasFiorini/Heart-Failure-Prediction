...
customSummary <- function (data, lev = NULL, model = NULL){
  ...
  for(i in seq(0.1, 0.95, 0.05)){
    predi <- factor(ifelse(data[, "pos"] > i, "pos", "neg"))
    singlesens <- sensitivity(predi, data[, "obs"], "pos")
    senslist <- c(senslist, singlesens)
    
    cm <- table(data[,"obs"], predi)
...
skim(heart)
```
Note that there is no critical imbalance between the target variable distribution. That is important for the prediction phase.
...
age <- ggplot(heart, aes(x = HeartDisease, y = Age, group = HeartDisease, fill = HeartDisease)) + geom_boxplot(alpha=0.5) +
    ...
    ggtitle("Age influence for Heat Disease") + theme(plot.title = element_text(hjust = 0.5))
age
```
...
genderDist <- ggplot(heart, aes(x = Sex)) + geom_bar(stat="count", width=0.7, fill="steelblue")+
  theme_minimal() + ggtitle("Gender distribuition") + theme(plot.title = element_text(hjust = 0.5))
genderDist
```
...
df_hd_bar <- ggplot(df_hd, aes(x="", y=freq, fill = sex))+
  ...
  facet_grid(~ sex) + ylab("Percentage") + theme(plot.title = element_text(hjust = 0.5))
...
ggplot(chestPainWithHD, aes(area = count, fill = ChestPainType, label = ChestPainType)) +
  geom_treemap() +
  geom_treemap_text() + scale_fill_manual(breaks = chestPainWithHD$ChestPainType, values = c("#2CF6B3", "#ECB0E1","#C9DDFF", "#DE6C83")) + ggtitle("Chest Pain Types for people with Heart Disease") + theme(plot.title = element_text(hjust = 0.5))
```
...
ggplot(ChestPainWithNoHD, aes(area = count, fill = ChestPainType, label = ChestPainType)) +
  geom_treemap() +
  geom_treemap_text() + scale_fill_manual(breaks = ChestPainWithNoHD$ChestPainType, values = c("#2CF6B3", "#ECB0E1","#C9DDFF", "#DE6C83")) + ggtitle("Chest Pain Types for people with no Heart Disease") + theme(plot.title = element_text(hjust = 0.5))
```
...
ggplot(heart, aes(x=FastingBS, group = HeartDisease, fill = HeartDisease)) + geom_bar(stat = "count") + theme_minimal() +
  ggtitle("Fasting BS Distribuition") + theme(plot.title = element_text(hjust = 0.5))
```
...
ggplot(heart, aes(x=HeartDisease, y=Oldpeak, group = HeartDisease,  fill = HeartDisease)) + geom_boxplot() + theme_minimal() + scale_fill_manual(breaks = heart$HeartDisease, values = c("#C9DDFF", "#DE6C83")) + ylab("OldPeak") +
  ggtitle("Old Peak for Heart Disease") + theme(plot.title = element_text(hjust = 0.5))
```

The OldPeak variable has a significant change whether the person has or not heart problems. 

```{r}
ggplot(heart, aes(x=ExerciseAngina, fill = HeartDisease)) + geom_bar() + ggtitle("Exercise Angina for Heart Disease") + theme(plot.title = element_text(hjust = 0.5))
```
...
Not good to see frequency distribuition...
...
Better now. Not that much influence over heart disease class.

```{r}
ContCoef(heart$HeartDisease, heart$RestingECG, correct = TRUE)
```
The correlation looks low and the plots shows similar frequency distribution for both groups on that variable. 
...
Input the potential most important features:
```{r}
...
par(pty = "s")
roc(test_reg$HeartDisease, predict_reg, plot=TRUE, legacy.axes=TRUE, percent = TRUE,
    xlab="False Positive Rate Percentage", ylab="True Positive Percentage Rate", col="red", print.auc=TRUE)
```

```{r message=FALSE, warning=FALSE}
roc.info <- roc(test_reg$HeartDisease, predict_reg, legacy.axes=TRUE)
roc.df <- data.frame(
...
roc.df %>% filter(roc.df$tpp > 90, roc.df$tpp < 95)
```

As the topic is sensitive and false negatives are dangerous, decreasing the threshold is important. Making it larger can clearly decrease TP percentage, making FN percentage increase. Therefore, fixing it to $0.1$ shows a good choice to take: 

```{r}
result <- ifelse(predict_reg > 0.1, 1, 0)
confMatrix <- table(test_reg$HeartDisease, result)
confMatrix
```

With only one False Negative, recall increases significantly and accuracy drops. But that is a more realistic prediction for this scenario.
...
predicted.data <- data.frame(
  probability.of.hd = predict_reg, 
  hd = test_reg$HeartDisease)
...
The chart gives a good logistic regression overview, as it shows the $0.1$ threshold and and the values that were predicted being above or below it (just a cooler way to see confusion matrix).
...
library(caret)
set.seed(123)
levels(heart$HeartDisease) <- c("neg", "pos")
...
For maximum Sensitivity, the code tries to find the threshold for that optimal measure. Therefore, after running this K-Fold, we can see that we are able to see that we maintain acceptable accuracy and precision while preserving Sensitivity rule for our case.
